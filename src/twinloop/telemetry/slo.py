...
class SLOEvaluator:
    ...
    def evaluate(self, metrics: TickMetrics) -> dict[str, ServiceSLO]:
        ...
        for sid in metrics.service_p95:
            sample = metrics.service_p95[sid]
            p95_ms = None if sample is None else sample * 1000.0
            availability = 1.0 - metrics.service_drop_rate[sid]
            p95_ok = (p95_ms is not None and metrics.service_throughput.get(sid, 0.0) > 0
                      and p95_ms <= self.config.p95_target_ms)
            availability_ok = availability >= self.config.availability_target
            compliant = p95_ok and availability_ok
            near_p95 = p95_ms is not None and p95_ms >= self.config.at_risk_fraction * self.config.p95_target_ms
